 ...
 */
AsyncSerialPort5* AsyncSerialPort5::portHandle = NULL;
...
void setParityPort5(USART_InitTypeDef& iUsartInitStruct, Parity iParity);
void setStopBitPort5(USART_InitTypeDef& iUsartInitStruct, StopBits iStopBit);
void setWordlengthPort5(USART_InitTypeDef& iUsartInitStruct, DataBits iWordLength);
void setLinkModePort5(GPIO_InitTypeDef& iGpioInitStruct,
		USART_InitTypeDef& iUsartInitStruct, LinkMode iLinkMode);
void setNvicPort5(uint8_t iPreempPriority, uint8_t iSubPriority);

/*
 * Constructors
 */
AsyncSerialPort5::AsyncSerialPort5(void)
{
	...
	preempPriority = SERIAL_PORT5_PREEMP_PRIORITY;
	subPriority = SERIAL_PORT5_SUBPRIORITY;
	interruptSetting = SERIAL_INT_DISABLE;

	dataStreamIn.Create(SERIAL_PORT5_BUFFERS_LENGTH, sizeof(uint8_t));
	dataStreamOut.Create(SERIAL_PORT5_BUFFERS_LENGTH, sizeof(uint8_t));

	portMutex.Create();
}

AsyncSerialPort5::AsyncSerialPort5(const AsyncSerialPort5&)
{
...
void AsyncSerialPort5::operator=(const AsyncSerialPort5&)
{
 ...
 * Instance getter
 */
AsyncSerialPort5 *AsyncSerialPort5::getInstance(void)
{
	if (AsyncSerialPort5::portHandle == NULL)
	{
		AsyncSerialPort5::portHandle = new AsyncSerialPort5;
	}

	return AsyncSerialPort5::portHandle;
}

/*
 * Serial port initializer
 */
SerialStatus AsyncSerialPort5::portInit(Parity iParityConf,
		...
		InterruptSetting iInterruptSetting)
{
	...
	if ((dataStreamIn.IsValid()) && (dataStreamOut.IsValid()) &&
			(portMutex.IsValid()))
	{
		initPort5(parity, stopBits, dataBits, linkMode, baudRate,
 ...
 */
SerialStatus AsyncSerialPort5::getCurrentStatus(void)
{
	return currentStatus;
}
 ...
 */
void AsyncSerialPort5::setCurrentStatus(SerialStatus iStatus)
{
 ...
 * ***USED ONLY IN INTERRUPT HANDLER***
 */
CQueue& AsyncSerialPort5::getOutStream(void)
{
	return dataStreamOut;
}

CQueue& AsyncSerialPort5::getInStream(void)
{
	return dataStreamIn;
}
 ...
 * Port reading method
 */
SerialStatus AsyncSerialPort5::getChar(const int8_t* oCharacter,
 ...
 */
SerialStatus AsyncSerialPort5::putChar(const int8_t iCharacter,
		portTickType iBlockTime)
{
	if (currentStatus == SERIAL_OK)
	{
...
SerialStatus AsyncSerialPort5::putString(const int8_t* const iString,
 ...
 * Mutex method
 */
portBASE_TYPE AsyncSerialPort5::takeMutex(portTickType iBlockTime)
{
	return portMutex.Take(iBlockTime);
}

portBASE_TYPE AsyncSerialPort5::giveMutex(void)
{
 ...
 * Closing port method
 */
void AsyncSerialPort5::closePort(void)
{
	delete AsyncSerialPort5::portHandle;
	AsyncSerialPort5::portHandle = NULL;
}
		...
		uint8_t iSubPriority, InterruptSetting iInterruptSetting)
{
	...
	setLinkModePort5(gpioInitStruct, usartInitStruct, iLinkMode);

	//set up the parity
	setParityPort5(usartInitStruct, iParity);

	//set up thestop bit config
	setStopBitPort5(usartInitStruct, iStopBit);

	//set up the word length
	setWordlengthPort5(usartInitStruct, iDataLength);
	...
	{
		setNvicPort5(iPreempPriority, iSubPriority);
...
void USART5_IRQHandler(void)
{
	static AsyncSerialPort5* portHandle = AsyncSerialPort5::getInstance();
